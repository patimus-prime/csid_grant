---
title: "Enhancing Population Prevalence Estimation of CSID and Improving Variant Annotation through Integrated Computational and Genetic Analysis"
...
Using this protein language model we can predict the delteriousness of missense mutations. The model outputs a score for each missense mutation, and an arbitrary threshold for deleterious/benign mutations is reported[@brandesGenomewidePredictionDisease2023]. The graph below is constructed using gnomAD data for missense variants and this model:
...
Overall, the population of predicted missense mutations that may be deleterious is XXX. To validate these results, a good approach may be to start from the most deleterious predicted mutation and work up the scores.
...
This is THE PLOT. We see that, yes, the benign tend to be greater than -7.5; but there are more benign samples below the -7.5 threshold than there are pathogenic samples, and overall we have very limited data here to work with. The threshold can't be proven in our circumstance to be useful.
...
And... yeah, ColabFold not capable of the 1800 residue complex. Only likely possible in OpenFold/AlphaFold with better machine. And... idk if it'd be possible to validate can just model the SI component, would miss some of the interactions. Maybe though.
